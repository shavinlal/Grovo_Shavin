...
class CreateTagVideoLesson():
    
    def createVideoLessontag(self):
        print "Reading data from excel sheet"
        book=xlrd.open_workbook(os.path.join('TestData.xlsx'))
        first_sheet = book.sheet_by_name('CreateTag')
        print("Fetching the Attribute Name from Excel Sheet\n")
        # read a cell
        cell = first_sheet.cell(27,2)
        TagName = cell.value
        print TagName
        
        cell = first_sheet.cell(27,3)
    ...
    def createTagForVideoLesson(self):
        try :
            obj2= CreateTagVideoLesson()
            obj2.createVideoLessontag() 
        finally:  
